...
export const climatiqApi = {
  ...
  async freight(params: FreightParams): Promise<EmissionResult> {
    try {
      ...
      switch (params.transport_mode) {
        case "road":
          emission_factor =
            "freight_vehicle-vehicle_type_truck-fuel_source_diesel-distance_na-weight_na";
          break;
        case "rail":
        ...
        case "sea":
          emission_factor =
            "freight_ship-vessel_type_na-fuel_source_na-route_type_na";
          break;
        case "air":
          emission_factor =
            "freight_flight-route_type_na-distance_na-weight_na";
          break;
        default:
          emission_factor =
            "freight_vehicle-vehicle_type_truck-fuel_source_diesel-distance_na-weight_na";
      ...
      switch (params.energy_type) {
        case "electricity":
          emission_factor = `electricity-energy_source_grid_mix${
            params.country ? `-country_${params.country}` : ""
          }`;
          break;
        case "natural_gas":
  ...
  async cbem(params: CBEMParams): Promise<EmissionResult> {
    try {
      ...
      const emission_factor =
        productMap[params.product] || "industrial_process-type_average";
  ...
  energy_local(
    ...
    const factor =
      emissionFactors[energy_type] || emissionFactors["electricity"];
